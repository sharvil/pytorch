...
class TestPatternMatcher(TestCase):
    ...
    def test_match_with_mutation(self):
        counter = 0
        test_pass = PatternMatcherPass(prevent_match_across_mutations=True, pass_name="test")
        ...
        with unittest.mock.patch(
            "torch._inductor.fx_passes.pre_grad.config.pre_grad_fusion_options", {"test": {}},
        ), unittest.mock.patch(
            "torch._inductor.fx_passes.pre_grad.PRE_GRAD_FUSIONS", [],
        ), unittest.mock.patch(
            "torch._inductor.fx_passes.pre_grad.PRE_GRAD_PATTERNS", {"test": test_pass},
        ):
